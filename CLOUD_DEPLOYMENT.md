...
- DOCX 실제 업로드
- DOC 실제 업로드 및 LibreOffice 기반 DOCX 자동 변환
- DOCX 문단/표 행 추출
- R2 원본 영구 저장 및 원본 보기
- D1 프로젝트/조항/판정 영구 저장
- 프로젝트 목록/상세/일괄검토
...
구형 `.doc`은 Railway의 LibreOffice로 `.docx`에 자동 변환한 뒤 동일한 분석 절차를 사용합니다. Sites 단독 Worker에서는 변환하지 않고 `/relay`를 통해 Railway 분석 서버에 전달합니다.
